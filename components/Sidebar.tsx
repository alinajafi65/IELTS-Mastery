...
import { LayoutDashboard, BookOpen, Library, Award, LogOut, ChevronRight, RefreshCw } from 'lucide-react';
...
const Sidebar: React.FC<SidebarProps> = ({ currentView, onNavigate, user, onLogout, onChangeType }) => {
  const menuItems = [
    ...
    { id: 'vault', label: 'Vocab Vault', icon: Library },
  ];
        ...
        {menuItems.map((item) => {
          ...
          return (
            ...
            >
              ...
              {item.id === 'vault' && user.vocabVault?.length > 0 && (
                <span className={`text-[9px] font-black px-2 py-0.5 rounded-full ${isActive ? 'bg-blue-600' : 'bg-slate-100'}`}>
                  {user.vocabVault.length}
                </span>
              )}
      ...
      <div className="p-8">
        <div className="bg-slate-50 p-6 rounded-[2rem] mb-6 border border-slate-100">
          ...
          <div className="flex items-end justify-between">
            ...
            <button onClick={onChangeType} className="flex flex-col items-end cursor-pointer hover:opacity-70 transition-opacity">
        ...
        <button onClick={onLogout} className="w-full flex items-center justify-center gap-3 px-6 py-4 text-xs font-black text-red-500 hover:bg-red-50 rounded-2xl transition-all uppercase tracking-widest active:scale-95">
